(function ($) {
    ...
    ls.merge = function () {
        ...
        if (chks.length <= 1) {
            ...
            if (window.confirm("Are you sure you want to merge selected session?")) {
                $.ajax({
                    type: "post",
                    dataType: "json",
                    url: lsAjax.ajaxurl,
                    data: { action: "ls_submission", type: "merge", nonce: lsAjax.nonce, post_id: lsAjax.post_id, merges: merges },
                    success: function (response) {
                        if (response.response === 'ok') {
                            for (var update in response.session) {
                                var id = response.session[update][0];
                                var name = ls.escape_html(response.session[update][1]);
    ...
    ls.set_time = function (cell, time) {
        ...
        if (time == "--") {
            ...
            if (date1.getDay() === date2.getDay() && date1.getMonth() === date2.getMonth() && date1.getYear() === date2.getYear()) {
                ...
                fmt = Math.floor((date2 - date1) / 1000 / 60 / 60 / 24) + " days";
            }
        }
        var c = cell;
    ...
    ls.mark = function (learnstone, response) {
        ...
        if (response >= 0) {
            $("input[data-menu=ls_menu" + learnstone + "]").each(function () { $(this).removeClass(lsAjax.LS_STYLES).addClass(LS_CLASSES[response]); });
        }
        if (response >= 0 || vals.length > 0) {
            $.ajax({
                ...
                dataType: "json",
                url: lsAjax.ajaxurl,
                data: { action: "ls_submission", type: "mark", post_id: lsAjax.post_id, learnstone: lsAjax.lss[learnstone], response: response, nonce: lsAjax.nonce, inputs: obj },
                success: function (response) {
                    if (response.response != 'ok') {
    ...
    ls.set_selected_index = function (index) {
        $('input[data-menu=ls_menu' + ls.getSelectedIndex() + ']').each(function () {
    ...
    ls.db_toggle_col = function (obj, tag, col) {
        ...
        while (!exit) {
            if (td2.hasClass('ls_db_input_hide')) {
                td2.removeClass("ls_db_input_hide").addClass("ls_db_input_show");
            }
            else if (td2.hasClass('ls_db_input_show')) {
                td2.removeClass("ls_db_input_show").addClass("ls_db_input_hide");
            }
            else {
                exit = true;
            }
    ...
    ls.db_input_update = function (td, selected, updatelink) {
        ...
        if (txt && txt.length != 0) {
            ...
            if (url == 1) {
                ...
                if (qm == -1) {
                    ...
                    op = encodeURI(op.substring(0, qm - 1)) + "?" + encodeURIComponent(op.substring(qm + 1));
                }
        ...
        if (updatelink) {
            ...
            if (td.data('input').length == 0) {
                ...
                while ((tdtest.hasClass('ls_db_input_show') || tdtest.hasClass('ls_db_input_hide')) && full == 1) {
            ...
            while (tdtest2.hasClass('ls_db_input_show') || tdtest2.hasClass('ls_db_input_hide')) {
                tdtest2 = tdtest2.prev("td");
            }
            if (full == 1) {
                tdtest2.find("a").removeClass('ls_db_input_a_empty').addClass('ls_db_input_a_full');
            } else {
                tdtest2.find("a").removeClass('ls_db_input_a_full').addClass('ls_db_input_a_empty');
            }
        }
    }
    ...
    ls.db_format = function (selected) {
        ...
        $.ajax({
            ...
            dataType: "json",
            url: lsAjax.ajaxurl,
            data: { action: "ls_submission", type: "format", field: selected.attr('name'), format: selected.val(), post_id: lsAjax.post_id, nonce: lsAjax.nonce },
            success: function (response) {
                //alert(response.response);
            }
        });
        ...
        if (response.response == "ok") {
            ...
            for (var update in response.updates) {
                for (var resp in response.updates[update]) {
                    $("#" + update + resp).removeClass().addClass('ls_respspan ls_resp' + response.updates[update][resp]);
                }
            }
    ...
    ls.submission = function (email) {
        $.ajax({
            ...
            dataType: "json",
            url: lsAjax.ajaxurl,
            data: { action: "ls_submission", type: "sub", email: email, post_id: lsAjax.post_id, nonce: nonce },
            success: function (response) {
                //alert(response.response);
            }
        });
    ...
    ls.login = function () {
        $.ajax({
            ...
            dataType: "json",
            url: lsAjax.ajaxurl,
            data: { action: "ls_submission", type: "login", post_id: lsAjax.post_id, username: $('.ls_username').eq(0).val(), password: $('.ls_password').eq(0).val() },
            success: function (response) {
                if (response.loggedin) {
                    lsAjax.login_warn = 0;
                    $(".ls_loginphide").removeClass("ls_loginpshow").addClass("ls_loginphide");
                    ...
                    ele.removeClass("ls_loginmenushow").addClass("ls_loginmenuhide");
                    ele2.removeClass("ls_loginmenuhide").addClass("ls_loginmenushow");
                    var ls;
                    for (ls = 0; ls < lsAjax.lss.length; ls++) {
                        var resp = response.lss['ls_' + lsAjax.lss[ls]];
                        if (resp) {
                            $("input[data-menu=ls_menu" + ls + "]").removeClass(lsAjax.LS_STYLES).addClass(resp);
                        }
                    }
    ...
    ls.move_up = function () {
        ...
        if (selected > 0) {
            $('#ls_content ul li').eq(selected - 1).before($('#ls_content ul li').eq(selected));
        }
    }
    ...
    ls.move_down = function () {
        ...
        if (selected < $('input[name=ls_selection]').length - 1) {
            $('#ls_content ul li').eq(selected + 1).after($('#ls_content ul li').eq(selected));
        }
    }
    ...
    ls.add_handlers = function () {
        $('#ls_results input[data-item]').click(function (e) {
            e.preventDefault();
            var selected = ls.get_selected();
            var id = $(this).data("item");
            var title = $('span[data-item=' + id + ']').html();
            var li = "<li class='ls_course_item_edit'><input name='ls_item[]' type='hidden' value='" + id + "' /><input name='ls_selection' type='radio' value='" + id + "' checked='checked'/>" + title + "</li>";
            if (selected == -1) {
    ...
    ls.login_warn = function () {
        if (lsAjax.login_warn > 0) {
            $(".ls_loginphide").removeClass("ls_loginphide").addClass("ls_loginpshow");
        }
    }
    ...
    ls.db_highlight = function (col, highlight) {
        if (col > 0) {
            if (highlight) {
                $("#ls_dashboard tr").each(function () { $(this).find("td").eq(col).addClass("ls_hover") });
                $("#ls_dashboard tr").each(function () { $(this).find("th").eq(col).find("div").addClass("ls_hover") });
            }
            else {
                $("#ls_dashboard tr").each(function () { $(this).find("td").eq(col).removeClass("ls_hover") });
                $("#ls_dashboard tr").each(function () { $(this).find("th").eq(col).find("div").removeClass("ls_hover") });
            }
        }
    }
    ...
    ls.db_filter = function (filter) {
        ...
        if (filter != 2) {
            $('#ls_dashboard tr').each(function () {
                ...
                else if ((val == -1) || ($(this).data('class').indexOf('|' + val + '|') != -1)) {
                    $(this).show();
                    $(this).removeClass('ls_db_odd ls_db_even').addClass((line % 2) == 1 ? 'ls_db_odd' : 'ls_db_even'); $(this).show();
                    line++;
                }
                else {
                    $(this).hide();
        ...
        $('#ls_stream tr').each(function () {
            if (typeof ($(this).data('user')) !== 'undefined') {
                ...
                if ((val == -1) || (s.data('class').indexOf('|' + val + '|') != -1)) {
                    $(this).show();
                    $(this).removeClass('ls_db_odd ls_db_even').addClass((line % 2) == 1 ? 'ls_db_odd' : 'ls_db_even'); $(this).show();
                    line++;
                }
    ...
    .ready(function () {
        if (lsAjax.view_mode == 0) {
            $("#ls_slides div[data-menu^=ls_menu]").each(function () {
                $("#ls_slides input[data-menu=" + $(this).data("menu") + "item]").val($(this).find("h1").eq(0).text());
            });
            var i = 0;
            ...
            for (i = 0; i < LS_CLASSES.length; i++) {
                $(".ls_lightsal" + i).on("click", { value: i }, function (e) {
                    e.preventDefault();
                    ls.mark(ls.getSelectedIndex(), e.data.value);
                });
                $(".ls_lightsl" + i).on("click", { value: i }, function (e) {
                    e.preventDefault();
                    ls.mark(ls.getSelectedIndex(), e.data.value);
                });
            }
            ...
            if (lsAjax.login_warn > 0) {
                setTimeout(function () { ls.login_warn() }, lsAjax.login_warn);
            }
        }
        else if (lsAjax.view_mode == 3) {
            $('#ls_dashboard td').on('mouseover mouseleave', function (e) {
                ls.db_highlight($(this).index(), e.type == 'mouseover');
            });
            $('#ls_dashboard th').on('mouseover mouseleave', function (e) {
                ls.db_highlight($(this).index(), e.type == 'mouseover');
            });
            ...
            $('.postbox h3, .postbox .handlediv').off('click.postboxes');
            if (lsAjax.view_mode == 2 || lsAjax.view_mode == 4) {
                $("#ls_search_sub").click(function (e) { e.preventDefault(); ls.search(); });
                $("#ls_move_up").click(function (e) { e.preventDefault(); ls.move_up(); });
                $("#ls_move_down").click(function (e) { e.preventDefault(); ls.move_down(); });
                $("#ls_delete").click(function (e) { e.preventDefault(); ls.delete_item(); });
                $("#ls_bulk").click(function (e) { e.preventDefault(); ls.switch_mode(); });
                ls.add_handlers();
            }
        }
    });
